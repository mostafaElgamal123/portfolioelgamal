<?php 
use App\Models\Media; $media=Media::first();
use App\Models\Service; $service=Service::all();
?>
 <!-- ======= Footer ======= -->
  <footer id="footer" class="footer">

    <div class="container">
      <div class="row gy-4">
        <div class="col-lg-5 col-md-12 footer-info">
          <a href="index.html" class="logo d-flex align-items-center">
            <span>Impact</span>
          </a>
          <p>Cras fermentum odio eu feugiat lide par naso tierra. Justo eget nada terra videa magna derita valies darta donna mare fermentum iaculis eu non diam phasellus.</p>
          <div class="social-links d-flex mt-4">
              <a href="@if(isset($media->twitter)) {{$media->twitter}} @endif" class="twitter"><i class="bi bi-twitter"></i></a>
              <a href="@if(isset($media->facebook)) {{$media->facebook}} @endif" class="facebook"><i class="bi bi-facebook"></i></a>
              <a href="@if(isset($media->instagram)) {{$media->instagram}} @endif" class="instagram"><i class="bi bi-instagram"></i></a>
              <a href="@if(isset($media->linkedin)) {{$media->linkedin}} @endif" class="linkedin"><i class="bi bi-linkedin"></i></a>
          </div>
        </div>

        <div class="col-lg-2 col-6 footer-links">
          <h4>Useful Links</h4>
          <ul>
              <li><a href="{{route('index')}}">Home</a></li>
              <li><a href="{{route('about')}}">About</a></li>
              <li><a href="{{route('service')}}">Services</a></li>
              <li><a href="{{route('portfolio')}}">Portfolio</a></li>
              <li><a href="{{route('blog')}}">Blog</a></li>
              <li><a href="{{route('contact')}}">Contact</a></li>
          </ul>
        </div>

        <div class="col-lg-2 col-6 footer-links">
          <h4>Our Services</h4>
          <ul>
            @foreach($service as $servic)
            <li><a href="{{url('services/'.$servic->id)}}">{{$servic->title}}</a></li>
            @endforeach
          </ul>
        </div>

        <div class="col-lg-3 col-md-12 footer-contact text-center text-md-start">
          <h4>Contact Us</h4>
          <p>
            @if(isset($media->location)) {{$media->location}} @endif<br>
            <strong>Phone:</strong> @if(isset($media->phone)) {{$media->phone}} @endif<br>
            <strong>Email:</strong> @if(isset($media->email)) {{$media->email}} @endif<br>
          </p>

        </div>

      </div>
    </div>

    <div class="container mt-4">
      <div class="copyright">
        &copy; Copyright <strong><span>Impact</span></strong>. All Rights Reserved
      </div>
      <div class="credits">
        <!-- All the links in the footer should remain intact. -->
        <!-- You can delete the links only if you purchased the pro version. -->
        <!-- Licensing information: https://bootstrapmade.com/license/ -->
        <!-- Purchase the pro version with working PHP/AJAX contact form: https://bootstrapmade.com/impact-bootstrap-business-website-template/ -->
        Designed by <a href="https://bootstrapmade.com/">BootstrapMade</a>
      </div>
    </div>

  </footer>
  <!-- End Footer -->

  <a href="#" class="scroll-top d-flex align-items-center justify-content-center"><i class="bi bi-arrow-up-short"></i></a>

  <div id="preloader"></div>

  @include('web.front.layout.footer.vendors')
  @include('web.front.layout.footer.main-js')

